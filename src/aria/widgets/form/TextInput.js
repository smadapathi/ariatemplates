...
Aria
		.classDefinition({
			...
			$dependencies : [ "aria.utils.Function", "aria.utils.Data",
					"aria.utils.String",
			...
			$prototype : {
				...
				$init : function(p, def, sdef) {
					p.automaticallyBindedProperties = p.automaticallyBindedProperties
							.concat("prefillError");
				},
				...
				_getPrefilledText : function(value) {
					if (value && this.controller
						...
						return this.controller
								.getDisplayTextFromValue(this._cfg.prefill);
					} else {
						return value;
					}
				},
				...
				_inputWithFrameMarkup : function(out) {
					var cfg = this._cfg, skinObj = this._skinObj, hts = this._helpTextSet, htc = this._skinObj.helpText, color = this
							._getTextFieldColor();
					var stringUtils = aria.utils.String, MultiACPrfx = null;

					MultiACPrfx = [ '<ul class="xMultiAutocomplete_selected" style="padding:0;margin:0;"></ul>' ];

					var inlineStyle = [ 'padding:', skinObj.innerPaddingTop,
							'px ', skinObj.innerPaddingRight, 'px ',
							skinObj.innerPaddingBottom, 'px ',
							skinObj.innerPaddingLeft,
							'px;position:relative;margin:0;' ];
					if (!this._simpleHTML) {
						inlineStyle
								.push("background-color:transparent;border-width:0px;vertical-align:top;");
					}
					...
					if (hts) {
						...
						if (this._isIE7OrLess && this._isPassword) {
							...
							color = htc.color
									+ (htc.italics ? ";font-style:italic" : "");
						}
					}
					var type = this._isPassword && !hts ? "password" : "text";

					var inputWidth = this._frame.innerWidth
							- skinObj.innerPaddingLeft
							- skinObj.innerPaddingRight;
					if (inputWidth < 0) {
						inputWidth = 0;
					}
					...
					if (this._isTextarea) {
						out
								.write([
										'<textarea',
										Aria.testMode ? ' id="' + this._domId
												+ '_textarea"' : '',
										cfg.disabled ? ' disabled="disabled"'
										...
										'color:',
										color,
										';overflow:auto;resize:none;height: '
												+ this._frame.innerHeight
												+ 'px; width:',
										inputWidth,
										'px;"',
										'value=""',
										(cfg.maxlength > -1 ? 'maxlength="'
												+ cfg.maxlength + '" ' : ' '),
										(cfg.tabIndex != null ? 'tabindex="'
												+ this._calculateTabIndex()
												+ '" ' : ' '),
										spellCheck,
										'>',
										stringUtils
												.encodeForQuotedHTMLAttribute((this._helpTextSet) ? cfg.helptext
														: text), '</textarea>'
				...
				_initInputMarkup : function(elt) {
					...
					var inputIndex = !(this._isMultiAutocomplete) ? 0 : 1;
					this._textInputField = this._frame.getChild(inputIndex);
				...
				checkValue : function(arg) {
					...
					var hasErrors = (this._cfg.formatErrorMessages.length ? true
							: false);
					var report;
					...
					if (report.errorMessages.length) {
						if (!performCheckOnly) {
							...
							&& aria.utils.Type
									.isArray(this._cfg.formatErrorMessages)
					...
					if (report) {
						...
						if (!delayedValidation) {
							...
							if (text != null && !this._helpTextSet) { // if text was
								...
								// the display
								this.getTextInputField().value = arg.multiAuto ? ""
										: text;
								if (report.caretPosStart != null
										&& report.caretPosEnd != null) {
									this.setCaretPosition(report.caretPosStart,
											report.caretPosEnd);
								}
							}
							if (typeof value != 'undefined' && !stopValueProp
							...
							if (report.ok) {
								...
								if (hasFocus && report.matchCorrectValueStart) {
									...
									if (cfg.directOnBlurValidation) {
										this.changeProperty("formatError",
												false);
									}
								} else {
									if (cfg.directOnBlurValidation) {
										this
												.changeProperty("formatError",
														true);
									}
								}
				 ...
				 * @protected
				 */
				_onBoundPropertyChange : function(propertyName, newValue,
						oldValue) {
					if (propertyName === 'value') {
						...
						if (res.isValid) {
							if (this._cfg && res.report
									&& res.report.text != null) {
								if (this._cfg.directOnBlurValidation) {
									this.changeProperty("formatError", false);
								}
							...
							this.$logError(this.WIDGET_VALUE_IS_WRONG_TYPE,
									[ newValue ]);
							res.report.$dispose();
							return;
						}
						...
						this.setHelpText(true);
						if (((aria.utils.Type.isArray(cfg.value) && aria.utils.Array
								.isEmpty(cfg.value)) || !cfg.value)
						...
						this._reactToChange();
							...
							|| propertyName === 'formatError'
							|| propertyName === 'formatErrorMessages'
							|| propertyName === 'error'
						...
						this.$InputWithFrame._onBoundPropertyChange.apply(this,
								arguments);
					}
				},
				...
				_updateState : function() {
					...
					if ((this._isIE7OrLess || this._simpleHTML)
							&& !this._helpTextSet) {
						this.getTextInputField().style.color = this
								._getTextFieldColor();
					}
					if (!(this._cfg.formatError || this._cfg.error)) {
				...
				_dom_onfocus : function(event) {
					...
					if (!this._keepFocus) {
						...
						this._currentCaretPosition = null;
						if (caretPosition) {
							this.setCaretPosition(caretPosition.start,
									caretPosition.end);
						}
					}
							...
							if (cfg.directOnBlurValidation) {
								if (cfg.bind) {
									...
									if (bind) {
										...
										aria.utils.Data.validateValue(
												dataholder, name, null, groups,
												'onblur');
				...
				_setAutomaticBindings : function(cfg) {
					...
					if (value && value.inside) {
						metaDataObject = aria.utils.Data._getMeta(value.inside,
								value.to, false);
					...
					if (prefill && prefill.inside) {
						metaDataObject = aria.utils.Data._getMeta(
								prefill.inside, prefill.to, false);
				...
				_checkCfgConsistency : function() {
					...
					if (cfg.autoselect == null) {
						...
						cfg.autoselect = aria.widgets.environment.WidgetSettings
								.getWidgetSettings().autoselect;
					}
					...
					if (cfg.invalidText) {
						...
						// prefill is defined
						if ((aria.utils.Type.isArray(value) && aria.utils.Array
								.isEmpty(value))
								|| (!value && value !== 0)) {
							if (cfg.prefill && cfg.prefill + "") {
								this._isPrefilled = true;
				...
				setHelpText : function(enable) {
					...
					if (!helpText || !this._helpTextSet && !enable
							|| (this._hasFocus || this._keepFocus) && enable) {
						return;
					}
					...
					var color = enable ? helpTextConfig.color : this
							._getTextFieldColor();
					var fontStyle = enable && helpTextConfig.italics ? "italic"
							: "normal";
				...
				setPrefillText : function(enable, value, updateState) {
					...
					if (enable) {
						if (!field
								|| (field.value && !this._helpTextSet && !this._isPrefilled)) {
							return;
						}
